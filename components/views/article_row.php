...
<article id="post-8" class="post-<?=$v->id;?> post type-post status-publish format-standard hentry category-uncategorized tag-html tag-wordpress">
    ...
    <header class="entry-header">
        <h1><a href="<?=Url::to(['blog/view', 'id' => $v->id]);?>" rel="bookmark"><?=$v->title;?></a></h1>

        <p class="entry-meta">
            <ul class="list-inline entry-meta">
                <li class="meta-date"> Posted on <a href="<?=Url::to(['archives/view', 'year' => date('Y', strtotime($v->create_date)), 'month' => date('m', strtotime($v->create_date))]);?>" rel="date">
    ...
    <?=StringHelper::truncate($v->description, 200, '...', null, true);?>
    </div>
